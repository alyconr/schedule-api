...
def process_workbook(file_bytes: bytes, filename: str, import_type: str) -> dict[str, Any]:
            ...
            if len(rows) < 2:
                ...
                for r_idx in range(1, len(rows)):
                    ...
                    if nombre and str(nombre).strip():
                        ...
                        # Contract type
                        ct_clean = tipo_contrato.upper()
                        if "PLANTA" in ct_clean:
                            ct_enum = "planta"
                            default_h = 30
                        elif "CONTRATISTA" in ct_clean:
                            ct_enum = "contratista"
                            default_h = 40
                        else:
                            ct_enum = "otro"
                        ...
                        # Base Hours
                        try:
                            base_h = Decimal(str(horas_formacion)) if horas_formacion is not None else Decimal(default_h)
                        except Exception:
                            base_h = Decimal(default_h)
                            warnings.append(ImportIssue(
                                sheet=sheet_ia,
                                row=row_num,
                                entity="instructor",
                                severity="warning",
                                message=f"Horas de formación inválidas. Se usó valor por defecto: {default_h}",
                        ...
                        # Max Hours
                        try:
                            add_h = Decimal(str(horas_adicionales)) if horas_adicionales is not None else Decimal("0")
                            max_h = base_h + add_h
                        except Exception:
                            max_h = base_h
                        ...
                        instructor_dict = {
                            ...
                            "contract_type_name": ct_clean if tipo_contrato else "PLANTA"
                        }
    ...
    return {
        ...
        "items": {
            "instructors": instructors_list[:50],
            "environments": environments_list[:50],
            "groups": groups_list[:50],
            "programs": programs_list[:50],
            "learning_results": learning_results_list[:50]
        },
        "warnings": warnings,
        "errors": errors
    }
